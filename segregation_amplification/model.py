...
from segregation_amplification import core
...
class SegregationMaskingModel(object):

    def __init__(self, spatial_shape=None, seed=None, params=None):
        if spatial_shape is None:
            self.spatial_shape = (75, 150)
        else:
            self.spatial_shape = spatial_shape

        # All randomness in the model (placement, types, parameter draws, relocation and infection) flows from this one generator,
        # so a given seed reproduces a run exactly.
        self.seed = seed
        self.rng = np.random.default_rng(seed)
        self.c_rand_seed = int(self.rng.integers(0, 2 ** 31 - 1))
        ...
        self.initial_params = dict(DEFAULT_PARAMS)
    ...
    def initialize_agents(self):
        ...
        self.shuffled_positional_indices = self.get_shuffled_positional_indices(self.positional_indices)
    ...
    def get_shuffled_positional_indices(self, positional_indices):
        ...
        self.rng.shuffle(shuffled_positional_indices, axis=0)
    ...
    def initialize_agent_types(self):
        ...
        # Agent types are 1 = Blue (pro-masking), 2 = Red (anti-masking). 0 is left blank to reflect absence of agents.
        self.agent_types = self.rng.integers(1, 3, size=self.n_agents).astype(np.int32)
    ...
    def get_truncated_normal_parameter_map(self, min, max, mean, std, spatial_shape, agent_ids_map):
        a = stats.truncnorm.rvs((min - mean) / std, (max - mean) / std, loc=mean, scale=std, size=spatial_shape[0] * spatial_shape[1], random_state=self.rng)
    ...
    def update(self, n_iterations):
        ...
        core.update_model_arrays(self, np.int32(n_iterations))
        self.n_iterations_run += n_iterations

    def get_masking_rate(self):
        """Share of agents currently choosing to mask."""
        return float(np.sum(self.masking_choice == 1)) / self.n_agents

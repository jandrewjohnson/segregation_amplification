...
@cython.boundscheck(True)
@cython.wraparound(True)
def spatial_segregation_only(model, long n_iterations):

    # Extract model attributes to c-objects
    cdef long[::1] agent_ids = model.agent_ids
    cdef long[::, ::1] agent_locations = model.agent_locations
    cdef long[::, ::1] unoccupied_locations = model.unoccupied_locations
    cdef long[::1] agent_types = model.agent_types
    cdef long[::, ::1] agent_ids_map = model.agent_ids_map
    cdef long[::, ::1] types_map = model.types_map
 ...
 # allocate_all_sectors(ndarray[np.float64_t, ndim=2] projections_array not None, # n_sectors by n_countries float array of demand to allocate, will also be used to count down what is left and when a country/sector has finished.
...
#
#     return D
## FROM OPTIMIZATION RESULTS, this is much faster than above.
# @cython.boundscheck(False)
# @cython.wraparound(False)
...
@cython.boundscheck(True)
@cython.wraparound(True)
def spatial_segregation_only_non_memview(model, long n_iterations):
...
@cython.boundscheck(True)
@cython.wraparound(True)
def spatial_segregation_and_externality_memview(model, long n_iterations):
    ...
    cdef float[::, ::1] d_map = model.parameter_maps['d']
    cdef float[::, ::1] s_map = model.parameter_maps['s']
    cdef float[::, ::1] r_map = model.parameter_maps['r']
    cdef float[::, ::1] b_map = model.parameter_maps['b']
    cdef float[::, ::1] masking_choice = model.masking_choice
    ...
    cdef float[::, ::1] mean_metric_map = np.zeros([n_rows, n_cols], dtype=DTYPEFLOAT32)
    cdef float[::, ::1] average_reciprocal_response_from_masking = np.zeros([n_rows, n_cols], dtype=DTYPEFLOAT32)
    cdef float[::, ::1] sum_reciprocal_response_from_masking = np.zeros([n_rows, n_cols], dtype=DTYPEFLOAT32)
            ...
            for rd in range(-neighborhood_radius, neighborhood_radius + 1):
                for cd in range(-neighborhood_radius, neighborhood_radius + 1):
                    ...
                    if types_map[r, c] == types_map[neighborhood_r, neighborhood_c]:
                        if not (neighborhood_r == r and neighborhood_c == c):  # Make sure to not account for one's reciprocal response to themself.
                            sum_metric += 1
                            sum_reciprocal_response_from_masking[r, c] += r_map[r, c]


            mean_metric = sum_metric / n_in_neighborhood
            average_reciprocal_response_from_masking[r, c] = mean_metric

            #UHHH aren't these the same?
            mean_metric_map[r, c] = mean_metric
            ...
            for rd in range(-neighborhood_radius, neighborhood_radius + 1):
                for cd in range(-neighborhood_radius, neighborhood_radius + 1):
                    ...
                    if infection_status[neighborhood_r, neighborhood_c] > 0:
                        ...
                        current_infection_probability = infection_probability * (1. - 0.95 * (masking_choice[r, c]) / 2.)
